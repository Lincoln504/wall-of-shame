...
A) IF ARTICLE TEXT is provided (not UNAVAILABLE):
   0. STUB / ERROR PAGE CHECK — BEFORE ANYTHING ELSE: Read the article text and ask: is this actually the article about the topic, or is it a "Page Not Found" / 404 page, an access-denied message, a login/subscription wall, a site homepage, or content clearly unrelated to the title? If it is a stub or error page — set "valid": false immediately. Do NOT analyze the draft or use other context to compensate. Stub content is never a basis for a Wall of Shame entry.
   1. DIRECTIONAL CHECK: apply the directional test above using the real article text. If the article's own argument DEFENDS or NORMALIZES harm — proceed. If the article EXPOSES, CRITICIZES, or DOCUMENTS harm — set "valid": false immediately; do not proceed to grounding steps.
   2. VERIFY THE QUOTE: check whether the summary contains text inside quotation marks. If it does, confirm those exact words appear in the ARTICLE TEXT. If the quoted text does not appear verbatim in the ARTICLE TEXT, either (a) replace it with a real verbatim excerpt copied word-for-word from the ARTICLE TEXT, or (b) rephrase the sentence as a paraphrase without quotation marks. Never retain a quote you cannot confirm. Never invent a replacement quote — a paraphrase without quotes is always better than a fabricated quote.
   3. VERIFY THE CLAIMS: check every claim in the analysis against the ARTICLE TEXT; remove or soften anything the article does not support. Keep the critical verdict ONLY if the article genuinely exhibits the harmful framing.
   4. GROUND THE SPECIFICS: where the ARTICLE TEXT contains a concrete real detail that sharpens the analysis, use it (it is grounded, not invented). You MAY include a specific (a number, name, statute) ONLY if it literally appears in the ARTICLE TEXT.
