#!/usr/bin/env bash
#
# populate.sh — drive the Wall of Shame agent toward a target corpus size.
#
...
#     per round) so a leak in one round can't compound across many rounds.
#
...
LOG="agent/data/populate.log"   # *.log is gitignored
...
log "════════════════════════════════════════════════════════════════"
log "population START — target=$TARGET concurrency=$CONCURRENCY max_flat=$MAX_FLAT"
...
final="$(count)"
log "════════════════════════════════════════════════════════════════"
log "population FINISHED — $final findings (started at $start_count, +$((final - start_count)) over $round rounds)"
log "════════════════════════════════════════════════════════════════"
